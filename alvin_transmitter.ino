...
MotorCommand readGestureAndCalculateMotors() {
  ...
  int forwardSpeed = 0;
  if (filteredPitch > GESTURE_THRESHOLD) {
    forwardSpeed = map(constrain(filteredPitch, GESTURE_THRESHOLD, 45), 
                       GESTURE_THRESHOLD, 45, 100, 255);
  } else if (filteredPitch < -GESTURE_THRESHOLD) {
    forwardSpeed = map(constrain(filteredPitch, -45, -GESTURE_THRESHOLD), 
                       -45, -GESTURE_THRESHOLD, -255, -100);
  }

  int turnSpeed = 0;
  if (filteredRoll > TURN_THRESHOLD) {
    turnSpeed = map(constrain(filteredRoll, TURN_THRESHOLD, 45), 
                    TURN_THRESHOLD, 45, 50, 150);
  } else if (filteredRoll < -TURN_THRESHOLD) {
    turnSpeed = map(constrain(filteredRoll, -45, -TURN_THRESHOLD), 
                    -45, -TURN_THRESHOLD, -150, -50);
  }
